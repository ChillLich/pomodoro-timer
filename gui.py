...
class MyGUI:
    ...
    def _on_settings_changed(self, key: str, value):
        ...
        Перерисовка, вызывает обновление только нужных частей UI.
        """
    ...
    def _update_ui_config(self):
        """
        Обновляет конфигурацию UI (цвета, шрифты, видимость кнопок).
        """
        colors = self.settings.get_current_theme_colors()
        fonts = self.settings.get_current_fonts()
    ...
    def _update_all_frames_colors(self, widget, colors):
        ...
        for child in widget.winfo_children():
            if isinstance(child, tk.Frame):
                # определить какой цвет фона применить
    ...
    def _update_quick_settings_buttons(self, colors, fonts):
        ...
        # обновить и показать только видимые
        for key, sys_key in quick_settings_map.items():
            ...
            # если кнопка не видма она остаётся скрытой (pack_forget выше)
    ...
    def _create_quick_settings_section(self, parent, colors, fonts):
        ...
        for key, sys_key in quick_settings_map.items():
            ...
            # сохранить ссылку на кнопку независимо от видимости
            self.qs_buttons[key] = btn

            # показать только если кнопка должна быть видима
            if is_visible:
                btn.pack(side=tk.LEFT, padx=2, fill=tk.X, expand=True)
            else:
                btn.pack_forget()
    ...
    def _on_close(self):
        """
        Корректно закрывает приложение, останавливая таймер и аудио и завершая работы.
        """
        if self._tick_job:
            self.root.after_cancel(self._tick_job)
        if self._rebuild_job:
